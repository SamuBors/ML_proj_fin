...
beta.ridge <- solve(t(x)%*%x +lambda*diag(nrow(t(x)%*%x)))%*%t(x)%*%y #ridge coefficients
y_forecast <- x%*%beta.ridge #predicted y
date <- lead(dates,12)[1:(nrow(final_d)-12),]

#prediction plots
y_lim <- max(max(abs(y_forecast)),max(abs(y_random_walk)),max(abs(y)))
plot(y_forecast,type = "l", col = "red", ylim=c(-y_lim,y_lim), lty = 2,xaxt = "n",yaxt = "n",xlab = "",ylab = "") #ridge prediction
par(new = T)
...
legend("bottomright", 
       legend = c("Ridge forecasts", "Random walk forecast", "True series"), 
       col = c("red", "blue", "black"),
       lty=c(2,3,1),
       bg = "white",
       cex = 0.65) #legende size
title(main = paste("Ridge forecasts with lambda equal to",lambda,", random walk forecasts and true series"))
abline(h=0,col="grey") #zero line
par(new = F)

#rolling estimates
lambda <- 1 #lambda
lambda_f <- lambda/10000000 #lambda for the function
...
for (i in 1:(nrow(final)-132)){ #the number of repetition is T(=nrow(final)=564) - 120(=estimation period) - 12(=preidciton skip) +1(=from 11 to 20 there are 20-11+1 numbers)
  ...
  beta.ridge <- solve(t(x_train)%*%x_train +lambda*diag(nrow(t(x_train)%*%x_train)))%*%t(x_train)%*%y_train #ridge coefficients
  y_pred[i] <- x_test%*%beta.ridge #predicted y
  ridge.fit.t <- glmnet(as.matrix(x_train), as.matrix(y_train), lambda=lambda_f, intercept=T, standardize = F, alpha = 0) #ridge through the command
  y_pred1[i] <- predict(ridge.fit.t, as.matrix(x_test), standardize=F, alpha = 0) #prediction through the command
...
plot(y_pred,type = "l", col = "red", ylim=c(-y_lim,y_lim), lty = 2,xaxt = "n",yaxt = "n",xlab = "",ylab = "") #ridge prediction
par(new = T)
...
gg<-data1 %>% 
  ...
  scale_color_manual(values = c("red", "blue", "black"), labels=c("Ridge forecasts", "Random walk forecasts", "True series"))+
  scale_linetype_manual(values = c("dashed", "dotted", "solid"), labels=c("Ridge forecasts", "Random walk forecasts", "True series"))+
...
plot_filename <- "forecast_ridge.png"
ggsave(file.path(plot_directory, plot_filename), plot = gg, width = 9, height = 4, dpi= 1000)

#SFE and SFE_rw
SFE <- (y_pred-y_ts)^2 #squared forecasting errors of the ridge
SFE_rw <- (y_rw-y_ts)^2 #squared forecasting errors of the rw
MSFE <- mean(SFE) #MSFE of the ridge
MSFE_rw <- mean(SFE_rw) #MSFE of the rw
MSFE_relative <- MSFE/MSFE_rw #relative MSFE

#SFE plots
y_lim <- 0.005
plot(y_dates,SFE,type = "l", col = "red", ylim=c(0,y_lim),xaxt = "n",yaxt = "n",xlab = "",ylab = "") #ridge squared errors
par(new = T)
...
#big loop
#rolling estimates
l <- 10^seq(-6,6,2) #lambda grid
MSFE_relative <- matrix(NA,nrow=0,ncol=1)
y_pred <- matrix(NA,nrow=(nrow(final)-120-12),ncol=length(l))
y_ts <- matrix(NA,nrow=0,ncol=1)
y_rw <- matrix(NA,nrow=0,ncol=1)
y_dates <- matrix(NA,nrow=0,ncol=1)
for (i in 1:(nrow(final)-120-12)){ #the number of repetition is T(=nrow(final)=564) - 120(=rolling window timeframe) - 12(=preidciton skip) +1(=from 11 to 20 there are 20-11+1 numbers)
  ...
  for (k in l){
    m <- m+1
    beta.ridge <- solve(t(x_train)%*%x_train +k*diag(nrow(t(x_train)%*%x_train)))%*%t(x_train)%*%y_train #ridge coefficients
    y_pred[i,m] <- x_test%*%beta.ridge #predicted y
  }
  y_ts[i] <- y_final[132+i,] #storing the true series, the test dependent variable
  y_rw[i] <- y_final[120+i,] #storing the true series lagged 12 month, for comparison
  y_dates[i] <- as.Date(dates[132+i,]) #storing the prediction dates
...
#SFE and SFE_rw
SFE <- (y_pred-y_ts)^2 #squared forecasting errors of the ridge
MSFE <- colMeans(SFE) #MSFE of the ridge
SFE_rw <- (y_rw-y_ts)^2 #squared forecasting errors of the rw
MSFE_rw <- mean(SFE_rw) #MSFE of the rw
MSFE_relative <- MSFE/MSFE_rw #relative MSFE
plot(log10(l),(MSFE_relative), type="l",xlab = "Log lambda",ylab = "MSFE relative")

#plot
data1 <- data.frame(l = l, MSFE_relative = MSFE_relative)
gg <- ggplot(data1, aes(x = log10(l), y = MSFE_relative)) +
  geom_line() +
  labs(x = "Log Lambda", y = "MSFE relative")

# Save the plot
plot_directory <- "/Users/samueleborsini/Library/CloudStorage/GoogleDrive-sam.sb74@gmail.com/Il mio Drive/ML project/R/plots"
plot_filename <- "MSFE_ridge.png"
ggsave(file.path(plot_directory, plot_filename), plot = gg, width = 6, height = 4, dpi = 1000)

#storing results
y_pred_ridge <- y_pred
MSFE_relative_ridge <- MSFE_relative
lambda_ridge <- l